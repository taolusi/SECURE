...
from classes import EventMention, EntityMention
from extraction_utils import load_ECB_plus, load_fcc_documents, order_docs_by_topics
...
parser.add_argument(
    ...
    default="configs/build_features_config.json",
)
parser.add_argument(
    "--output_path",
    type=str,
    help="The path to output folder (Where to save the processed data)",
    default="data/ecb+/processed",
)
...
def have_string_match(mention, arg_str, arg_start, arg_end):
    ...
    """
    if (
        mention.mention_str == arg_str and mention.start_offset == arg_start
    ):  # exact string match + same start index
        return True
    if mention.mention_str == arg_str:  # exact string match
        return True
    if (
        mention.start_offset >= arg_start and mention.end_offset <= arg_end
    ):  # the argument span contains the mention span
        return True
    if (
        arg_start >= mention.start_offset and arg_end <= mention.end_offset
    ):  # the mention span contains the mention span
        return True
    if (
        len(
            set(mention.tokens_numbers).intersection(set(range(arg_start, arg_end + 1)))
        )
        > 0
...
def find_argument(
    rel_name, rel_tokens, matched_event, sent_entities, sent_obj, is_gold, srl_obj
):
    """
                ...
                if sent_srl_info is not None:
                    for event_srl in sent_srl_info.srl:
                        ...
                        for event_mention in sent_events:
                            if (
                                event_ecb_tok_ids == event_mention.tokens_numbers
                                or event_text == event_mention.mention_str
                                or event_text in event_mention.mention_str
                                or event_mention.mention_str in event_text
                            ):
                                event_found = True
...
def match_entity_with_srl_argument(
    sent_entities, matched_event, srl_arg, rel_name, is_gold
):
    """
    This function matches between an argument of an event mention and an entity mention.
    ...
    for entity in sent_entities:
        if (
            srl_arg.ecb_tok_ids == entity.tokens_numbers
            or srl_arg.text == entity.mention_str
            or srl_arg.text in entity.mention_str
            or entity.mention_str in srl_arg.text
        ):
            if rel_name == "AM-TMP" and entity.mention_type != "TIM":
                continue
            if rel_name == "AM-LOC" and entity.mention_type != "LOC":
    ...
    if set_pred_mentions:
        ...
        for event in event_mentions:
            set_elmo_embed_to_mention(
                event, avg_sent_embeddings
            )  # set the head contextualized vector

        for entity in entity_mentions:
            set_elmo_embed_to_mention(
                entity, avg_sent_embeddings
            )  # set the head contextualized vector
...
def load_data(config_dict):
    ...
    if "ECB" in config_dict["corpus"]:
        train_datasets.append(
            load_ecb_gold_data(
                config_dict["ecb+_train_text_file"],
                config_dict["ecb+_train_event_mentions"],
                config_dict["ecb+_train_entity_mentions"],
            )
        )
        dev_datasets.append(
            load_ecb_gold_data(
                config_dict["ecb+_dev_text_file"],
                config_dict["ecb+_dev_event_mentions"],
                config_dict["ecb+_dev_entity_mentions"],
            )
        )
        test_datasets.append(
            load_ecb_gold_data(
                config_dict["ecb+_test_text_file"],
                config_dict["ecb+_test_event_mentions"],
                config_dict["ecb+_test_entity_mentions"],
            )
        )
...
def main(args):
    ...
    if config_dict["load_elmo"]:  # load ELMo embeddings
        elmo_embedder = ElmoEmbedding(
            config_dict["options_file"], config_dict["weight_file"]
        )
